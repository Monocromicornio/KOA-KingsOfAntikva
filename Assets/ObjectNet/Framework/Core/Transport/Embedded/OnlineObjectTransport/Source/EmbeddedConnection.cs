...
namespace com.onlineobject.objectnet.embedded {
    ...
    public abstract class EmbeddedConnection
    {
        ...
        private bool _canTimeout = true;
        /// <summary>Whether or not the connection can be disconnect.</summary>
        ...
        protected EmbeddedConnection()
        {
            ...
            AvgSendAttemptsResilience = TransportDefinitions.AvgSendAttemptsResilience;
            MaxSendAttempts = TransportDefinitions.ReliableSendAttempts;
        ...
        public ushort Send(EmbeddedMessage message, bool shouldRelease = true)
        {
            ...
            {
                // Update variable just in case of developper updated in running time
                MaxSendAttempts = TransportDefinitions.ReliableSendAttempts;
                AvgSendAttemptsResilience = TransportDefinitions.AvgSendAttemptsResilience;
        ...
        internal bool HandleReliableData(ushort sequenceId, Queue<MessageToHandle> messagesToHandle, DataReceivedEventArgs e)
        {
            return reliable.HandleReliableData(sequenceId, messagesToHandle, e);
        }
        ...
        private void UpdateSendAttemptsViolations() {
            if ( TransportDefinitions.WasDisconnectionValuesChanged() ) {
                // Update variable just in case of developper updated in running time
                sendAttemptsViolations = 0;
                Metrics.RollingReliableSends.Reset();
            }
        ...
        private class ReliableSequencer : Sequencer {
            ...
            /// <summary>
            /// Store tha last valid id received from peer
            /// </summary>
            private ushort lastValidId = 0;

            /// <inheritdoc/>
            internal ReliableSequencer(EmbeddedConnection connection) : base(connection) { }

            Dictionary<ushort, MessageToHandle> StoredMessages = new Dictionary<ushort, MessageToHandle>();
            ...
            internal bool HandleReliableData(ushort sequenceId, Queue<MessageToHandle> messagesToHandle, DataReceivedEventArgs e) {
                ...
                if (sequenceGap != 0) {
                    ...
                    if (sequenceGap > 0) {
                        //store all the valid messages to the dictionary so it can be executed in order
                        Buffer.BlockCopy(e.DataBuffer, 1, message.Data, 1, e.Amount - 1);
                        this.StoredMessages[sequenceId] = new MessageToHandle(message, header, e.FromConnection);

                        // The received sequence ID is newer than the previous one
                        if (sequenceGap > 64)
                            EmbeddedLogger.Log(LogType.Warning, this.connection.Peer.LogName, $"The gap between received sequence IDs was very large ({sequenceGap})!");

                        this.receivedSeqIds.ShiftBy(sequenceGap);
                        this.lastValidId = sequenceId;
                    } else {
                        // The received sequence ID is older than the previous one (out of order message)
                        sequenceGap = -sequenceGap;
                    }

                    doHandle = !this.receivedSeqIds.IsSet(sequenceGap);
                    this.receivedSeqIds.Set(sequenceGap);
                }

                //the message is valid.. try executing all the messages in sequnce after the last message was added
                if (doHandle) {
                    ushort nextSequenceID = (ushort)((lastReceivedSeqId + 1) % ushort.MaxValue);

                    while (this.StoredMessages.ContainsKey(nextSequenceID)) {
                        messagesToHandle.Enqueue(StoredMessages[nextSequenceID]); //execute
                        this.StoredMessages.Remove(nextSequenceID);
                        this.lastReceivedSeqId = nextSequenceID;
                        nextSequenceID = (ushort)((nextSequenceID + 1) % ushort.MaxValue);
                    }
                }

                this.connection.SendAck(sequenceId, this.lastValidId, this.receivedSeqIds);
                return doHandle;
            }
