...
namespace com.onlineobject.objectnet.embedded {
    ...
    internal class EmbeddedPendingMessage
    {
        ...
        {
            if (TransportDefinitions.IsDisconnectionValuesChanged()) {
                sendAttempts = 0;
            }

            if (sendAttempts >= connection.MaxSendAttempts && connection.CanQualityDisconnect && connection.CanDisconnect)
            {
